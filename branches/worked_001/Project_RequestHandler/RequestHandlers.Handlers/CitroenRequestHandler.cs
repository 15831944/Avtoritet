...
{
	public class CitroenRequestHandler : IRequestHandler
	{
		...
		public bool NeedAuthorization(string url, CookieContainer cookieContainer)
		{
			...
			if (cookies.Count > 0)
			{
				CitroenRequestHandler.PrintCookies(cookies);
			}
		...
		{
			await HttpProxyServer.SendRequest(CitroenRequestFactory.CreateLogoutRequest(), cookieContainer);
		}
		...
		{
			return await HttpProxyServer.SendRequest(CitroenRequestFactory.CreateLoginRequest(this.login, this.password), cookieContainer);
		}
