...
namespace PushUpdater
{
 public partial class MainWindow
 {
  ...
  private void CheckTempFolderForUpdate()
  {
   ...
   if (DirectoryHasFiles(SourceDirName))
   {
    ...
    if (DirectoryClear(SourceDirName))
    {
     ...
     if (!CanStartProcessAsAdministrator()) return;
    }
    else
    {
     if (File.Exists(ZipName))
  ...
  private static void DirectoryCopy(string sourceDirName, string destDirName, bool copySubDirs)
  {
   ...
   if (!string.IsNullOrEmpty(destDirName) && !Directory.Exists(destDirName))
   {
