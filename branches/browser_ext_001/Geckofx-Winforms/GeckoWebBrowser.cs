...
namespace Gecko
{
 ...
 /// </summary>
 public partial class GeckoWebBrowser : IGeckoWebBrowser, nsIWebBrowserChrome, nsIContextMenuListener2, nsIWebProgressListener, nsIWebProgressListener2, //nsIWebBrowserChromeFocus, -- TODO
  nsIInterfaceRequestor, nsIEmbeddingSiteWindow, nsIDOMEventListener, nsISHistoryListener, nsITooltipListener, nsIObserver, nsIHttpActivityObserver, nsISupportsWeakReference //nsIWindowProvider,
 {
  #region Fields
  ...
  /// </summary>
  private Dictionary<string, Action<string>> _messageEventListeners = new Dictionary<string, Action<string>>();
  ...
  private class WindowCreator : nsIWindowCreator2
  {
   ...
   {
//    chromeFlags = (int) (GeckoWindowFlags.WindowBorders | GeckoWindowFlags.WindowClose | GeckoWindowFlags.WindowResize); // 

    // for chrome windows, we can use the AppShellService to create the window using some built-in xulrunner code
    var flags = (GeckoWindowFlags) chromeFlags;
  ...
  private nsIClipboardCommands ClipboardCommands
  {
   get
   {
    return _ClipboardCommands ?? (_ClipboardCommands = Xpcom.QueryInterface<nsIClipboardCommands>(WebBrowser));
   }
  }
  ...
  public bool CanCopyImageContents
  {
   get
   {
    return CanPerform(ClipboardCommands.CanCopyImageContents);
   }
  }
  ...
  public bool CanCopyImageLocation
  {
   get
   {
    return CanPerform(ClipboardCommands.CanCopyImageLocation);
   }
  }
  ...
  public bool CanCopyLinkLocation
  {
   get
   {
    return CanPerform(ClipboardCommands.CanCopyLinkLocation);
   }
  }
  ...
  public bool CanCopySelection
  {
   get
   {
    return CanPerform(ClipboardCommands.CanCopySelection);
   }
  }
  ...
  public bool CanCutSelection
  {
   get
   {
    return CanPerform(ClipboardCommands.CanCutSelection);
   }
  }
  ...
  public bool CanPaste
  {
   get
   {
    return CanPerform(ClipboardCommands.CanPaste);
   }
  }
  ...
  private nsICommandManager CommandManager
  {
   get
   {
    return _CommandManager ?? (_CommandManager = Xpcom.QueryInterface<nsICommandManager>(WebBrowser));
   }
  }
  ...
  public bool CanUndo
  {
   get
   {
    return CommandManager.IsCommandEnabled("cmd_undo", null);
   }
  }
  ...
  public bool CanRedo
  {
   get
   {
    return CommandManager.IsCommandEnabled("cmd_redo", null);
   }
  }
  ...
  public GeckoDocument Document
  {
   get
   {
    return DomDocument as GeckoDocument;
   }
  }
  ...
  public void SaveDocument(string path, string outputMimeType) //hatton added mimeType param
  {
   ...
   if (persist != null)
   {
    persist.SaveDocument(DomDocument._domDocument, (nsISupports) Xpcom.NewNativeLocalFile(path), null, outputMimeType, 0, 0);
   }
   else
   {
  ...
  void nsIContextMenuListener2.OnShowContextMenu(uint aContextFlags, nsIContextMenuInfo info)
  {
   ...
   if (!this.NoDefaultContextMenu)
   {
    ...
    {
     optionals.Add(new MenuItem("Undo", delegate
      {
       Undo();
      }));
     optionals.Add(new MenuItem("Redo", delegate
      {
       Redo();
      }));
    ...
    {
     optionals.Add(new MenuItem("Back", delegate
      {
       GoBack();
      }));
     optionals.Add(new MenuItem("Forward", delegate
      {
       GoForward();
      }));
    ...
    if (this.CanCopyImageContents)
     optionals.Add(new MenuItem("Copy Image Contents", delegate
      {
       CopyImageContents();
      }));

    if (this.CanCopyImageLocation)
     optionals.Add(new MenuItem("Copy Image Location", delegate
      {
       CopyImageLocation();
      }));

    if (this.CanCopyLinkLocation)
     optionals.Add(new MenuItem("Copy Link Location", delegate
      {
       CopyLinkLocation();
      }));

    if (this.CanCopySelection)
     optionals.Add(new MenuItem("Copy Selection", delegate
      {
       CopySelection();
      }));

    MenuItem mnuSelectAll = new MenuItem("Select All");
    mnuSelectAll.Click += delegate
     {
      SelectAll();
     };
    ...
    mnuOpenInSystemBrowser.Click += delegate
     {
   ...
   GeckoContextMenuEventArgs e = new GeckoContextMenuEventArgs(PointToClient(MousePosition), menu, associatedLink, backgroundImageSrc, imageSrc, GeckoNode.Create(Xpcom.QueryInterface<nsIDOMNode>(info.GetTargetNodeAttribute())));
...
#warning when geckoFX is used in WPF application there are no forms. We should rewrite this code
  ...
  public event EventHandler SecurityStateChanged
  {
   add
   {
    this.Events.AddHandler(SecurityStateChangedEvent, value);
   }
   remove
   {
    this.Events.RemoveHandler(SecurityStateChangedEvent, value);
   }
  }
  ...
  public List<string> ActiveNetworkChannelUrls
  {
   get
   {
    return activeNetworkChannelUrls;
   }
  }
  ...
  public void ObserveActivity(nsISupports aHttpChannel, UInt32 aActivityType, UInt32 aActivitySubtype, Int64 aTimestamp, UInt64 aExtraSizeData, nsACStringBase aExtraStringData)
  {
