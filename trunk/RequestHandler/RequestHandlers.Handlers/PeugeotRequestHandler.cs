...
{
	public class PeugeotRequestHandler : IRequestHandler
	{
		...
		public bool NeedAuthorization(string url, CookieContainer cookieContainer)
		{
			...
			if (cookies.Count > 0)
			{
				PeugeotRequestHandler.PrintCookies(cookies);
			}
		...
		{
			await HttpProxyServer.SendRequest(PeugeotRequestFactory.CreateLogoutRequest(), cookieContainer);
		}
		...
		{
			return await HttpProxyServer.SendRequest(PeugeotRequestFactory.CreateLoginRequest(this.login, this.password), cookieContainer);
		}
