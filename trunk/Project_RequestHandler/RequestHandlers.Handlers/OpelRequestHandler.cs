...
{
	internal class OpelRequestHandler : IRequestHandler
	{
		...
		public bool NeedAuthorization(string url, CookieContainer cookieContainer)
		{
			...
			if (cookies.Count > 0)
			{
				OpelRequestHandler.PrintCookies(cookies);
			}
		...
		{
			return await HttpProxyServer.SendRequest(OpelRequestFactory.CreateLoginRequest(this.login, this.password), cookieContainer);
		}
