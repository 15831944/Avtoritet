        ...
        public MainWindow()
        {
            try
            {
                ...
                Task.Factory.StartNew(delegate
                {
                    ...
                    if (commandLineArgs[1].Contains("opel"))
                    {
                        base.Dispatcher.BeginInvoke(new Action(delegate
                        {
                            base.Title = "Opel Dealer Online";
                        }), new object[0]);
                        using (FileStream fileStream = new FileStream("Session_Opel.txt", FileMode.Open, FileAccess.Read))
                        {
                    ...
                    if (commandLineArgs[1].Contains("chevrolet"))
                    {
                        base.Dispatcher.BeginInvoke(new Action(delegate
                        {
                            base.Title = "Chevrolet Dealer Online";
                        }), new object[0]);
                        using (FileStream fileStream = new FileStream("Session_Chevrolet.txt", FileMode.Open, FileAccess.Read))
                        {
                            using (StreamReader streamReader = new StreamReader(fileStream))
                            {
                                value = streamReader.ReadToEnd();
